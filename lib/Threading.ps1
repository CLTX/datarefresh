function ForEach-Parallel {
    param(
        ...
        [PSObject]$AdditionalInputObject,      
        [Parameter(Mandatory=$false)]
        [string[]]$Modules,      
        [Parameter(Mandatory=$false)]
        [int]$MaxThreads=5
    ...
    BEGIN {
        ...
        $iss.ImportPSSnapIn("VMware.VimAutomation.Core", [System.Management.Automation.PSReference] $psSnapinException)

        $pool = [Runspacefactory]::CreateRunspacePool(1, $maxthreads, $iss, $host)
        
        $pool.open()
        $threads = @()
        #$ScriptBlock = $ExecutionContext.InvokeCommand.NewScriptBlock($Scriptblock.ToString())
    }
    PROCESS {
        $powershell = [powershell]::Create().addscript($scriptblock).addargument($InputObject).addargument($AdditionalInputObject)
...
function Invoke-Parallel {
    ...
    Begin {
        ...
        #region Init
            ...
            $sessionstate.ImportPSSnapIn("VMware.VimAutomation.Core", [System.Management.Automation.PSReference] $psSnapinException)
